...
DROP TRIGGER IF EXISTS update_biding_history;
...
CREATE TRIGGER insert_biding_history
    ...
    FOR EACH ROW
BEGIN
    INSERT INTO biding_history (bid_no, sal_no, addr_cd, addr_name, bid_cd, buyer_id, price, reason, bid_state, bid_date, grant_state, grant_date, first_date, first_id, last_date, last_id)
    VALUES (NEW.no, NEW.sal_no, NEW.addr_cd, NEW.addr_name, NEW.bid_cd, NEW.buyer_id, NEW.price, NEW.reason, NEW.bid_state, NEW.bid_date, NEW.grant_state, NEW.grant_date, NEW.first_date, NEW.first_id, NEW.last_date, NEW.last_id );
END//
DELIMITER ;
...
CREATE TRIGGER update_biding_history
    AFTER UPDATE ON biding
    FOR EACH ROW
BEGIN
    INSERT INTO biding_history (bid_no, sal_no, addr_cd, addr_name, bid_cd, buyer_id, price, reason, bid_state, bid_date, grant_state, grant_date, first_date, first_id, last_date, last_id)
    VALUES (NEW.no, NEW.sal_no, NEW.addr_cd, NEW.addr_name, NEW.bid_cd, NEW.buyer_id, NEW.price, NEW.reason, NEW.bid_state, now(), NEW.grant_state, NEW.grant_date, NEW.first_date, NEW.first_id, NEW.last_date, NEW.last_id );
END//
DELIMITER ;

-- -- AFTER DELET E trigger for sale table
-- DELIMITER //
...
--     VALUES (OLD.no, OLD.addr_cd, OLD.addr_name, OLD.bid_cd, OLD.buyer_id, OLD.price, OLD.reason, OLD.bid_date, OLD.grant_state, OLD.grant_date, OLD.bid_state, now(), OLD.first_date, OLD.first_id, OLD.last_date, OLD.last_id );
-- END//
-- DELIMITER ;
